...
<template>
    <div class="sidebar dark:bg-coal-600 bg-light border-e border-e-gray-200 dark:border-e-coal-100 fixed z-20 hidden lg:flex flex-col items-stretch shrink-0"
        data-drawer="true" data-drawer-class="drawer drawer-start top-0 bottom-0" data-drawer-enable="true|lg:false"
        data-drawer-backdrop-static="false" data-drawer-persistent="false" id="sidebar" ref="sidebarDrawer">
        <!-- Sidebar Header -->
        <SidebarHeader />
        ...
        <div class="sidebar-content flex grow shrink-0 py-5 pe-2" id="sidebar_content">
            <div class="scrollable-y-hover grow shrink-0 flex ps-2 lg:ps-5 pe-1 lg:pe-3" data-scrollable="true"
                ...
                <div class="menu flex flex-col grow gap-0.5" data-menu="true" data-menu-accordion-expand-all="false"
                    id="sidebar_menu">
                    <template v-for="(menu, index) in sidebarStore.menus" :key="index">
                        <div v-if="!menu.items" class="menu-item" :class="{
                            active: route().current(menu.route),
                        }" data-drawer-dismiss="true">
                            <a class="menu-label border border-transparent gap-[10px] ps-[10px] pe-[10px] py-[6px]"
                                href="#" @click="sidebarStore.setSelected(menu.title, 0)" tabindex="0">
                                <span class="menu-icon items-start text-gray-500 dark:text-gray-400 w-[20px]">
                                    <i class="ki-filled text-lg" :class="menu.icon">
                                    </i>
                                </span>
                                <span
                                    class="menu-title text-sm font-medium text-gray-800 menu-item-active:text-primary menu-link-hover:!text-primary hover:text-primary">
                                    {{ menu.title }}
                                </span>
                            </a>
                        </div>
                        <div v-else class="menu-item show menu-item-accordion" :class="{
